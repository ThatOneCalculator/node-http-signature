...
module.exports = {
	...
	parseRequest: function parseRequest(request, options) {
		...
		let headers = [request.headers["x-date"] ? "x-date" : "date"];
		...
		const authz =
			request.headers[authzHeaderName] ||
			request.headers[utils.HEADER.AUTH] ||
			request.headers[utils.HEADER.SIG];
		...
		let i = 0;
		let state =
			authz === request.headers[utils.HEADER.SIG] ? State.Params : State.New;
		...
		if (!parsed.params.algorithm) parsed.params.algorithm = "rsa-sha256";
		...
		for (i = 0; i < parsed.params.headers.length; i++) {
			...
			if (h === "request-line") {
				if (!options.strict) {
					 ...
					 */
					parsed.signingString += `${request.method} ${request.originalUrl} HTTP/${request.httpVersion}`;
				} else {
				...
				parsed.signingString += `(request-target): ${request.method.toLowerCase()} ${
					request.originalUrl
				}`;
			} else if (h === "(keyid)") {
		...
		if (request.headers.date || request.headers["x-date"]) {
			...
			if (skew > options.clockSkew * 1000) {
				throw new ExpiredRequestError(
					`clock skew of ${skew / 1000}s was greater than ${
						options.clockSkew
					}s`,
				);
			}
		...
		if (parsed.params.expires) {
			const expiredSince =
				Math.floor(Date.now() / 1000) - parsed.params.expires;
		...
		if (
			options.algorithms &&
			options.algorithms.indexOf(parsed.params.algorithm) === -1
		)
			throw new InvalidParamsError(
