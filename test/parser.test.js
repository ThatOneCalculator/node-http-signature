...
test("created in future", function (t) {
	...
	options.headers.Authorization = `Signature keyId="f,oo",algorithm="RSA-sha256",created=${created},headers="(created) dIgEsT",signature="digitalSignature"`;
	options.headers.digest = uuid();
	http.get(options, function (res) {
		t.equal(res.statusCode, 200);
		t.end();
...
test("expires expired", function (t) {
	...
	options.headers.Authorization = `Signature keyId="f,oo",algorithm="RSA-sha256",expires=${expires},headers="(expires) dIgEsT",signature="digitalSignature"`;
	options.headers.digest = uuid();
	http.get(options, function (res) {
		t.equal(res.statusCode, 200);
		t.end();
...
test("valid created and expires with skew", function (t) {
	...
	options.headers.Authorization = `Signature keyId="f,oo",algorithm="RSA-sha256",created=${created},expires=${expires},headers="(created) (expires) dIgEsT",signature="digitalSignature"`;
	options.headers.digest = uuid();
	http.get(options, function (res) {
		t.equal(res.statusCode, 200);
		t.end();
...
test("valid explicit headers request-line", function (t) {
	...
	http.get(options, function (res) {
		...
		res.on("end", function () {
			...
			t.equal(parsed.params.algorithm.toUpperCase(), parsed.algorithm);
			t.end();
		});
	});
});
